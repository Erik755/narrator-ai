...
/** Stores the user's Gemini key encrypted at rest with Android Keystore. */
public final class GeminiKeyStore {
    ...
    public static String maskedStatus(Context context) {
        String key = load(context);
        if (key.isEmpty()) return "Gemini: no configurado";
        String suffix = key.length() <= 4 ? "••••" : key.substring(key.length() - 4);
        return "Gemini: clave guardada · ••••" + suffix;
    }
