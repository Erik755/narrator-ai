...
# -----------------------------------------------------------------------------
# LocalLanguageAgent becomes a hybrid router: Gemini first when configured,
# LiteRT local model otherwise/fallback.
# -----------------------------------------------------------------------------
...
llm = replace_once(
    ...
    '''            if (closed || ready) return@execute
            ...
            val preferred = if (supportsEnhancedModel()) ENHANCED_MODEL else STANDARD_MODEL''',
    "skip local download when Gemini is configured",
)

anchor = '''        // Explicit, high-confidence device commands are safer and more reliable through
...
'''
replacement = '''        // Explicit, high-confidence device commands are safer and more reliable through
        // the deterministic router. Gemini/local LLM handle paraphrases and conversation.
        if (fallback.type != IntentAgent.Type.GENERAL && fallback.confidence >= 0.80) {
            callback.onResult(Result(fallback.type, fallback.argument, "", fallback.confidence, false))
            return
        }
...
'''
llm = replace_once(llm, anchor, replacement, "Gemini primary routing")
...
'''
llm = replace_once(llm, old_local, new_local, "local model helper extraction")
...
# -----------------------------------------------------------------------------
# Give voice interpretation the same rich Android context as typed commands.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Main screen: runtime API-key entry. Nothing is committed or embedded in APK.
# -----------------------------------------------------------------------------
...
print("patch_v26: Gemini primary brain + runtime key UI applied")
